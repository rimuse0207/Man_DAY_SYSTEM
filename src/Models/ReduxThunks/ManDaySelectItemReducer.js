...
export const AllManDayItemfetchData = () => {
    return async dispatch => {
        ...
        try {
            const response = await Request_Get_Axios('/API/PLM/Getting_data_for_item'); // Axios API 호출
...
const ManDaySelectItemReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case FETCH_DATA_FAILURE:
            return { ...state, loading: false, error: action.payload, Equipment_Lists_data: [], divide_Lists_data: [] };
        default:
            return state;
    }
};
