...
const PersonTableLists = ({ onClose, NowSelect }) => {
    ...
    const Getting_Department_Users = async () => {
        const Getting_Department_Users_Axios = await Request_Get_Axios('/API/PLM/Getting_Department_Users', {
    ...
    const Handle_Clicks_User = Selected => {
        dispatch(Insert_Man_Day_Select_Reducer_State_Func({ ...Filter_State, name: { value: Selected.email, label: Selected.name } }));
        onClose(false);
    };
