...
const TeamMainPage = ({ menuCode }) => {
    ...
    return (
        <PersonMainPageMainDivBox>
            <CommonFilters menuCode={menuCode} Getting_Person_Bar_State={() => Getting_Team_Part_Bar_State()}></CommonFilters>
            <div>
                <h2 style={{ textAlign: 'center' }}>
                    {' '}
                    {TextInfo?.itemName
                        ? `${TextInfo?.divideType === 'PART' ? '파트 : ' : '팀 : '}${TextInfo?.itemName} 공수정보`
                        : ''}{' '}
                </h2>
