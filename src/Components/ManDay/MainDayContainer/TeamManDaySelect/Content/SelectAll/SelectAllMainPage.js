...
const SelectAllMainPage = () => {
  ...
  const Getting_Man_Day_Info_Data = async (Select_Filter) => {
    ...
    const Getting_Man_Day_Info_Data_Axios = await Request_Post_Axios(
  ...
  const Develop_Operate_Excel_Download = async () => {
    const Develop_Operate_Excel_Download_Axios = await Request_Post_Axios(
  ...
  const dusckrnqns = (selected) => {
    ...
    const [a] = User_Rank.filter((list) => {
      if (
        Number(selected) >= list.min_rank &&
        Number(selected) <= list.max_rank
      ) {
        return list.name;
      }
    });
