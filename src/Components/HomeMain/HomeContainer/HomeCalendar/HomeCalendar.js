...
const HomeCalendar = () => {
    ...
    const Change_Color_State = async () => {
        const Getting_PIMS_Data = await Request_Get_Axios('/Ce_Route/Tools/Getting_PIMS_Select', { id: Login_Info.id });
    ...
    const getBarElements = (date, tdIndex) => {
        let events = Adata.filter(event => moment(date).isBetween(event.start_date, event.end_date, null, '[]'));
        ...
        events.sort((a, b) => new Date(a.start_date) - new Date(b.start_date) || new Date(a.end_date) - new Date(b.end_date));
            ...
            <div className="Calendar_Bar_Container">
                {events.map(event => {
                    ...
                    activeEvents = activeEvents.filter(activeEvent =>
                        moment(date).isBetween(activeEvent.start_date, activeEvent.end_date, null, '[]')
                    );
                    ...
                    let showTitle = !shownTitlesByDate[date] || !shownTitlesByDate[date].has(event.title);
    ...
    const handleMouseEnter = (e, event, tdIndex) => {
        ...
        requestAnimationFrame(() => {
            ...
            const containerRect = document.querySelector('.AnnualLeaveCalendarTableMainDivBox').getBoundingClientRect();
                ...
                <tr key={week} className="row">
                    {Array(7)
                        ...
                        .map((_, index) => {
                            ...
                            if (!tdRefs.current[index]) tdRefs.current[index] = React.createRef();
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {getBarElements(days.format('YYYY-MM-DD'), tdIndex)} {/* ✅ 수정된 부분 */}
                                </td>
                            );
                        })}
                </tr>
