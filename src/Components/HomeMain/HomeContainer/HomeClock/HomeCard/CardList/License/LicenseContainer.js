...
const LicenseContainer = () => {
    ...
    const Getting_Man_Day_Info_Befroe_Data = async () => {
        const Getting_Man_Day_Info_Before_Data_Axios = await Request_Get_Axios(`/API/PLM/Getting_Man_Day_Info_Before_Data`, {
            Select_Date: moment(Today_Date).clone().startOf('isoWeek').format('YYYY-MM-DD'),
        });
        if (Getting_Man_Day_Info_Before_Data_Axios.status) {
            if (Getting_Man_Day_Info_Before_Data_Axios.data.Date_Lists.length > 0) {
                setWeekContainer(Getting_Man_Day_Info_Before_Data_Axios.data.Date_Lists);
            }
        }
    };
    return (
        <LicnenseContainerMainDivBox>
            <div>
                <div style={{ marginBottom: '10px', fontWeight: 'bolder', textAlign: 'center', borderBottom: '1px solid lightgray' }}>
                    {' '}
