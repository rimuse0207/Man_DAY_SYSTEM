...
const AccessMainPage = () => {
  const [Access_Menu_Lists, setAccess_Menu_Lists] = useState(initialMenuList);
  ...
  return (
    <DepartmentMainPageMainDivBox>
      <UserHeader
        title={"권한 관리"}
        subDescript={"각 서비스 어드민의 관리자를 설정합니다."}
      ></UserHeader>
      ...
      <AccessMainPageMainDivBox>
        <div className="Full_Container">
                ...
                return (
                  <li style={{ marginBottom: `50px` }} key={list.name}>
                    <div
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        const checkeds = Access_Menu_Lists.map((pre) =>
                          pre.name === list.name
                            ? { ...pre, checked: !isChecked }
                            : pre,
                        );
                        setAccess_Menu_Lists(checkeds);
                      }}
                    >
                      <span className="Icon_Container">
                        {isChecked ? <CgLoadbar /> : <FaPlus />}
                      </span>
                      <span>{list.name}</span>
                    </div>
                    <div>
                          ...
                          list.children.map((item) => {
                            return (
                              <div
                                className="Menu_Container"
                                style={
                                  item.menu_select_code ===
                                  Now_Select_Menu?.menu_select_code
                                    ? {
                                        paddingLeft: "20px",
                                        color: "blue",
                                        fontWeight: "bolder",
          ...
          <div className="Right_Container">
            {Now_Select_Menu && (
              <AccessLists Now_Select_Menu={Now_Select_Menu}></AccessLists>
            )}
          </div>
        </div>
      </AccessMainPageMainDivBox>
